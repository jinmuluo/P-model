...
def pypmodel(tc, vpd, co2, fapar, ppfd, patm=np.NaN, elv=np.NaN, beta=146.0, soilm=1.0,
              meanalpha=1.0, apar_soilm=0.0, bpar_soilm=0.73300, c4='FALSE', method_optci='prentice14',
    ...
    # ------------------------------------------------------------------------------------------
    if do_ftemp_kphio == 'FALSE':
        kphio = 0.049977
    elif do_soilmstress == 'True':
        kphio = 0.087132
    else:
        kphio = 0.081785
    ...
    if elv == np.NaN and patm == np.NaN:
        print('Warning:Both elevation and atmospheric pressure are absent!')
